...
class Admin_notice {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_metsalescountdown_notices', [ $this, 'dismiss' ] );
	}

	public function dismiss() {

		if(empty($_POST['metsalescountdown_nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['metsalescountdown_nonce'])), 'metsalescountdown_nonce')) {
			wp_send_json_error();
		}
	...
	public function enqueue_scripts() {
		...
		?>
			...
			jQuery(document).ready(function ($) {
				$( '.metsalescountdown_notice.is-dismissible' ).on( 'click', '.notice-dismiss', function() {
					_this 		= $( this ).parents( '.metsalescountdown_active-notice' );
					...
					$.ajax({
						...
						type: 'POST',
						data: {
							action 	: 'metsalescountdown_notices',
							id 		: id,
							meta 	: meta,
							time 	: time,
							is_required: is_required,
							metsalescountdown_nonce: '<?php echo esc_html(wp_create_nonce( 'metsalescountdown_nonce' )) ?>'
						},
					});
	...
	public static function push($notice) {
		...
		$defaults = [
			...
			'show_if'          => true,
			'message'          => '',
			'class'            => 'metsalescountdown-active-notice',
			'dismissible'      => false,
			'btn'			   => [],
		...
		$classes = [ 'metsalescountdown_notice', 'notice' ];
		...
		// Notice ID.
		$notice_id    = 'metsalescountdown_sites_notice_id-' . $notice['id'];
		$notice['id'] = $notice_id;
		if ( ! isset( $notice['id'] ) ) {
			$notice_id    = 'metsalescountdown_sites_notice_id-' . $notice['id'];
			$notice['id'] = $notice_id;
	...
	public static function markup( $notice = [] ) {
		...
		<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo esc_attr( $notice['classes'] ); ?>">
			<p>
				<?php echo wp_kses($notice['message'], MetSalesCountdown\Helpers\Utils::get_kses_array()); ?>
			</p>

			<?php if(!empty($notice['btn'])):?>
			<p>
				<a title="<?php esc_html_e('Notification','met-sales-countdown')?>" href="<?php echo esc_url($notice['btn']['url']); ?>" class="button-primary"><?php echo esc_html($notice['btn']['label']); ?></a>
			</p>
			<?php endif; ?>
		</div>
		<?php
